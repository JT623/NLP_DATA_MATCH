...
def w2v(word, model):
    try:
        ...
        return np.zeros(args.char_embedding_size)
...
def w2v_process(vec):
    if len(vec) > args.seq_length:
        vec = vec[0:args.seq_length]
    elif len(vec) < args.seq_length:
        zero = np.zeros(args.char_embedding_size)
        length = args.seq_length - len(vec)
...
if __name__ == '__main__':
    load_all_data('../input/data/train.csv', data_size=None)
